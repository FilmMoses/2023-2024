...
const double distanceMultiplier = 0.88;
...
void default_constants() {
  ...
  //chassis.pid_turn_constants_set(1.9, 0, 13.5, 0);
...
void testAuto(){
  // chassis.pid_odom_injected_pp_set({{{0, 32}, fwd, 110},
  //                                   {{-24, 30}, fwd, 110}
  // });


  // chassis.pid_odom_injected_pp_set({{ {0, 10} , fwd, 110}});
  // chassis.pid_wait();

  // booper.move_absolute(200, 127);
  ...
  // chassis.pid_wait();
  // chassis.pid_odom_ptp_set({{24, 0}, fwd, 110});
  // chassis.pid_wait();

  // chassis.pid_odom_injected_pp_set({{{0, 24 * distanceMultiplier}, fwd, 110}});
  // chassis.pid_wait();
...
void wallReset(int duration){
  chassis.left_motors.at(0).move(-127);
  chassis.left_motors.at(1).move(-127);
  chassis.left_motors.at(2).move(-127);
  chassis.left_motors.at(3).move(-127);

  chassis.right_motors.at(0).move(-127);
  chassis.right_motors.at(1).move(-127);
  chassis.right_motors.at(2).move(-127);
  chassis.right_motors.at(3).move(-127);
  pros::delay(duration);
...
void runIntake(int speed){
  ...
  intakeMotor2.move(speed);
}

void runOuttake(int speed){
...
void rightMatchLoad(int xtimes){
  for (unsigned int i = 0; i < xtimes + 1; i++) {
...
void double180(){
 ...
 // rightWing.set_value(false);
  ...
  chassis.pid_odom_injected_pp_set({
                          {{12, 47}, fwd, DRIVE_SPEED},
                          });
  chassis.pid_wait();

  // TURN TO SPIT OUT TOWARDS LANE
  ...
  chassis.pid_odom_injected_pp_set({
                          {{13, 56}, fwd, DRIVE_SPEED},
                          });
  chassis.pid_wait();
  runOuttake(127);
  pros::delay(200);
  
  // TURN AND SPIT TOWARDS LANE 2nd
  runIntake(127);
  chassis.pid_odom_injected_pp_set({
                          {{13, 50}, fwd, DRIVE_SPEED},
                          });
  chassis.pid_wait();
  ...
  // BACK UP BACK TO THE MID LINE
  chassis.pid_odom_injected_pp_set({
                          {{13, 52}, rev, DRIVE_SPEED},
                          });
  chassis.pid_wait();
...
/*
//my code for exiting match loading
  chassis.pid_odom_smooth_pp_set({
                            {{1.5, 5}, fwd, 50}
...
void processingAuto(){
  ...
  // WING OUT TO CONTEST BALL TO LEFT
  leftWing.set_value(false);
  pros::delay(200);

  // SPIT OUT BALL TOWARDS LANE
  chassis.pid_turn_set(200, TURN_SPEED);
  chassis.pid_wait();
  runOuttake(127);
  pros::delay(400);

  // TURN BACK TOWARDS MID
  chassis.pid_turn_set(90, TURN_SPEED);
  chassis.pid_wait();
  ...
  */
  ...
  // MAYBE WALL RESET HERE!! (in case of contact during mid contest)

  // LINEUP FOR MATCH LOADING
  chassis.pid_odom_injected_pp_set({
                          {{-12, 6}, fwd, 110},
                          });
  ...
  // chassis.pid_wait();
  // chassis.pid_odom_injected_pp_set({
  //                             {{0, 72 * distanceMultiplier}, fwd, 110},
  //                             {{48, 32}, fwd, 110}
  //   });
